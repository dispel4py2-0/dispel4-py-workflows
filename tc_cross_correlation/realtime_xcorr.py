...
import numpy as np
from dispel4py.base import (BasePE, ConsumerPE, IterativePE,
...
class StoreToFile(ConsumerPE):
    ...
    def _process(self, data):
        xcorr1 = data[2]
        directory = ROOT_DIR + 'XCORR/' + starttime + '/' + str(
            data[0]) + '_' + str(data[1])
...
class Plot(ConsumerPE):
    ...
    def _process(self, data):
        xcorr1 = data[2]
        fout = ROOT_DIR + 'XCORR/%s_%s_%s.plot.png' % (self.filename, data[0],
                                                       data[1])
        sps = 4
        plt.plot(
            np.linspace((-len(xcorr1) + 1) / (2 * sps),
                        len(xcorr1) / (2 * sps), len(xcorr1)), xcorr1)
        plt.xlim(-1000, 1000)
        plt.savefig(fout)
