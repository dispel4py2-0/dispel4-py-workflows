...
from .normalization import env_norm, gain_norm, mean_norm, onebit_norm
            ...
            for station in f.readlines():
                if i % self.bulksize != 0:
                    ...
                    stations_list.append(
                        (network, station_name, "", self.channel, self.t_start,
                         self.t_finish))
                    i += 1
                else:
    ...
    def _get_waveforms(self, client, stations_list, stations_names):
        try:
            ...
            for tr in st:
                ...
                self.write('output',
                           [Stream(traces=[tr]), station_name, self.counter])
                self.counter += 1
            if stations_names:
...
def calc_norm(str1, norm, N):
    ...
    match norm:
        case 'onebit':
            # Method 1 - one-bit normalisation
            str1 = onebit_norm(str1)
        case 'mean':
            # Method 2 - moving-average normalisation
            str1 = mean_norm(str1, N)
        case 'gain':
            # Method 3 - 'triangle' normalisation
...
def calc_fft(str1, type, shift):
    if type is not None:
        ...
        print("station %s and network %s - size in calc_fft %s " %
              (str1[0].stats['station'], str1[0].stats['network'], size))
...
class PreTaskPE(IterativePE):
    ...
    def _process(self, data):
        ...
        try:
            ...
            self.log(traceback.format_exc())
            self.log("Getting an error in preprocess: %s-%s" %
                     (str1[0].stats['station'], str1[0].stats['network']))
            return None
